...
<form method="post" action="?action=anotacoes_edit&id=<?= (int)$anotacao->id ?>" 
  ...
  <label>Texto da Anotação<br>
    <textarea name="texto" required 
              style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);background:var(--surface-elev);color:var(--on-surface);min-height:100px"><?= htmlspecialchars($anotacao->texto) ?></textarea>
  </label><br><br>

  <label>ID do Usuário<br>
    <input type="number" name="user_id" required 
           value="<?= htmlspecialchars($anotacao->user_id) ?>" 
